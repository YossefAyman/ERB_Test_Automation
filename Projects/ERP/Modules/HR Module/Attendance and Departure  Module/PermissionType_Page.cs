using Automation_Testing;
using ERP_Automation_Testing;
using NUnit.Framework;
using OpenQA.Selenium;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;
using time = System.Threading.Thread;

namespace ERP_Automation_Testing
{
    public class PermissionType_Page
    {

        static IWebDriver Driver = Common.Driver;


        // Selectors

        static By Add_Button =                                       By.ClassName("btnAddItem");
        static By EmployeePermissionTypeName =                       By.Id("EmployeePermissionType_EmployeePermissionTypeName");
        static By EmployeePermissionTypeDescription =                By.Id("EmployeePermissionType_EmployeePermissionTypeDescription");
        static By Save_Button =                                      By.XPath("//input[@value='حفظ']");
        static By Search_TextBox =                                   By.ClassName("ng-valid");
        static By Search_Button =                                    By.ClassName("btn-light");
        static By NumOfItems_Text =                                  By.XPath("//*[@id=\"grid\"]/div[2]/div[2]/div/span");
        static By Edit_Button =                                      By.XPath("//i[@title='تعديل']");
        static By FirstItemDelete_Button =                           By.ClassName("btnDeleteItem");
        static By DeleteConfirm_Button =                             By.ClassName("confirm");




        public static void Goto()
        {
            Pages.PermissionTypePage();
            time.Sleep(2000);

        }

        public static void Add_PermissionType()
        {
            Driver.FindElement(Add_Button).Click();
            Driver.FindElement(EmployeePermissionTypeName).SendKeys(Data.M2HR.PermissionType_Name);
            Driver.FindElement(EmployeePermissionTypeDescription).SendKeys(Data.M2HR.PermissionType_Desc);
            Driver.FindElement(Save_Button).Click();
            time.Sleep(3000);
        }

        public static void Edit_PermissionType(string PermissionType_Name , string PermissionType_Desc)
        {
            Search(Data.M2HR.PermissionType_Name);
            Driver.FindElement(Edit_Button).Click();
            Driver.FindElement(EmployeePermissionTypeName).Clear();
            Driver.FindElement(EmployeePermissionTypeName).SendKeys(PermissionType_Name);
            Driver.FindElement(EmployeePermissionTypeDescription).Clear();
            Driver.FindElement(EmployeePermissionTypeDescription).SendKeys(PermissionType_Desc);
            Driver.FindElement(Save_Button).Click();
            time.Sleep(3000);
        }

        public static void Delete_PermissionType(string PermissionType_Name)
        {
            Search(PermissionType_Name);
            Driver.FindElement(FirstItemDelete_Button).Click();
            time.Sleep(1000);
            Driver.FindElement(DeleteConfirm_Button).Click();
            time.Sleep(2000);
        }

        public static string Search(string item)
        {
            Driver.FindElement(Search_TextBox).Clear();
            Driver.FindElement(Search_TextBox).SendKeys(item);
            Driver.FindElement(Search_Button).Click();
            time.Sleep(2000);

            if (Driver.FindElement(NumOfItems_Text).Text == "1 - 1 من 1")
            {
                return "Exist";
            }
            else if (Driver.FindElement(NumOfItems_Text).GetAttribute("class") == "ng-binding ng-hide")
            {
                return "NotExist";
            }
            else
            {
                return "Repeated";
            }
        }

    }
}