...
namespace ERP_Automation_Testing
{
    class BanReason_Page
    {
        ...
        public static void Add_BanReason()
        {
            Driver.FindElement(Add_Button).Click();
            Driver.FindElement(BanReasonName_Textbox).SendKeys(Data.Estates.BanReason_Name);
            Driver.FindElement(Save_Button).Click();
            time.Sleep(2000);
        }

        public static void Edit_BanReason(string Banreason, string BanReason_Name)
        {
            Search(Banreason);
            Driver.FindElement(EditFirstItem_Button).Click();
            Driver.FindElement(BanReasonName_Textbox).Clear();
            Driver.FindElement(BanReasonName_Textbox).SendKeys(BanReason_Name);
            Driver.FindElement(Save_Button).Click();
            time.Sleep(2000);
